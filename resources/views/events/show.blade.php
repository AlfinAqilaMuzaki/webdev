        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="flex justify-between items-start gap-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-1">
                                    ...
                                    {{ $event->lokasiRelasi ? $event->lokasiRelasi->nama_lokasi : $event->lokasi }}
                                </p>
            ...
            <aside class="lg:col-span-1">
                <div class="card sticky top-24 p-4 bg-base-100 shadow">
                    ...
                    <div class="mt-4">
                        <div class="flex justify-between text-sm text-gray-500"><span>Item</span><span id="summaryItems">0</span></div>
                        <div class="flex justify-between text-xl font-bold mt-1"><span>Total</span><span id="summaryTotal">Rp 0</span></div>
                    </div>
            ...
            <div class="modal-box">
                ...
                <div class="mt-4 space-y-2 text-sm">
                    ...
                    <div class="form-control w-full">
                        <label class="label">
                            <span class="label-text font-bold">Metode Pembayaran</span>
                        </label>
                        <select id="tipePembayaran" class="select select-bordered w-full" required>
                            <option value="" disabled selected>Pilih metode pembayaran</option>
                            @foreach($tipePembayarans as $tipe)
                            <option value="{{ $tipe->id }}" data-deskripsi="{{ $tipe->deskripsi }}">{{ $tipe->nama }}</option>
                            @endforeach
                        </select>
                        <div id="tipePembayaranDeskripsi" class="mt-2 p-3 bg-base-200 rounded-lg text-sm text-gray-600 hidden"></div>
                    </div>
                </div>
    ...
    <script>
        (function() {
            ...
            window.openCheckout = function() {
                ...
                tipePembayaranSelect.selectedIndex = 0;
                document.getElementById('tipePembayaranDeskripsi').classList.add('hidden');
            ...
            document.getElementById('tipePembayaran').addEventListener('change', function() {
                ...
                const deskripsiEl = document.getElementById('tipePembayaranDeskripsi');

                if (deskripsi) {
                    deskripsiEl.textContent = deskripsi;
                    deskripsiEl.classList.remove('hidden');
                } else {
                    deskripsiEl.classList.add('hidden');
                }
            });

            // --- LOGIC KONFIRMASI CHECKOUT (YANG DITAMBAHKAN) ---
            document.getElementById('confirmCheckout').addEventListener('click', async () => {
                ...
                const tipePembayaranId = document.getElementById('tipePembayaran').value;
                if (!tipePembayaranId) {
                ...
                try {
                    const res = await fetch("{{ route('orders.store') }}", {
                        ...
                        headers: {
                            ...
                            // Mengambil token CSRF dari meta tag header
                    ...
                    // redirect to orders list or payment
